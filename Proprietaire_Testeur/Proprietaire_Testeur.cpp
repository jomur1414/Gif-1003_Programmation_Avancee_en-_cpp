...
/**
 * @brief test de la méthode utilitaire vehiculeDejaPresent,
 * celle-ci est appelée dans le traitement effectué par ajouterVehicule.
 * cas valide : on ajoute un vehicule absent et aucune erreur n'est lancée.
 * cas invalide : on ajoute un vehicule et son NIV correspond déja a celui d'un
 * autre vehicule du proprietaire.
 */

TEST_F(unProprietaire, vehiculeDejaPresent)
{
Camion CamionUn("1HTMKADN43H561298", "L032006", 6500, 4);
	VehiculePromenade VehiculePromenadeUn("3VWFE21C04M000001", "IFT 006", 7);
	Camion CamionDeux("1HTMKADN43H561298", "L037777", 4500, 4);

ASSERT_NO_THROW(t_proprietaire.ajouterVehicule(VehiculePromenadeUn));

	ASSERT_NO_THROW(t_proprietaire.ajouterVehicule(CamionUn));

	ASSERT_THROW(t_proprietaire.ajouterVehicule(CamionDeux), VehiculeDejaPresentException);

	ASSERT_THROW(t_proprietaire.ajouterVehicule(VehiculePromenadeUn), VehiculeDejaPresentException);
}
